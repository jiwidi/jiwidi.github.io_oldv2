...
rTerm = function (options) {
	...
	this.callUpstart = function () {
		...
		if (visited) {
			this.callUpstartImmediately(this.data.upstart.slice(this
				.upcid));
		} else {
			setCookie("visited", 1, 5);
			for (cid in this.data.upstart) {
				if (cid > 0) {
					delay += (this.data.upstart[cid - 1].length + 1) *
						this.chartime;
				}


				setTimeout(function (aux) {
					if (window.blurred && !this
						.upstartInterrupted) {
						this.callUpstartImmediately(this.data
							.upstart.slice(this.upcid));
						this.upstartInterrupted = true;
						return;
					} else if (!this.upstartInterrupted) {
						this.enterCommand(this.data.upstart[this
							.upcid]);
					}
					this.upcid++;
				}, delay, cid);
		...
		delay += (this.data.upstart[this.data.upstart.length - 1]
			.length + 1) * this.chartime;
		return delay;
	};
	...
	this.callPhotos = function () {
		...
		for (cid in this.data.startphotos) {
			if (cid > 0) {
				delay += (this.data.startphotos[cid - 1].length + 1) *
					this.chartime;
			}
			setTimeout(function () {
				this.callUpstartImmediately(this.data
					.startphotos.slice(this.upcid));
			}, delay);
		}
		delay += (this.data.startphotos[this.data.startphotos.length -
			1].length + 1) * this.chartime;
		return delay;
	};
	...
	this.callKeyboard = function () {
		...
		for (cid in this.data.startkeyboards) {
			if (cid > 0) {
				delay += (this.data.startkeyboards[cid - 1].length +
					1) * this.chartime;
			}
			setTimeout(function () {
				this.callUpstartImmediately(this.data
					.startkeyboards.slice(this.upcid))
			}, delay);
		}
		delay += (this.data.startkeyboards[this.data.startkeyboards
			.length - 1].length + 1) * this.chartime;
		return delay;
	};
	...
	this.callBlog = function () {
		...
		for (cid in this.data.startblog) {
			if (cid > 0) {
				delay += (this.data.startblog[cid - 1].length + 1) *
					this.chartime;
			}
			setTimeout(function () {
				this.callUpstartImmediately(this.data
					.startblog.slice(this.upcid));
			}, delay);
		}
		delay += (this.data.startblog[this.data.startblog.length - 1]
			.length + 1) * this.chartime;
		return delay;
	};
	...
	this.updateTerm = function () {
		if (this.termPrev.split('$')[0].slice(-1) != "/") {
			this.termPrev = this.termPrev.split('$')[0] + '/$' + this
				.termPrev.split('$')[1]
		}
		this.termPrev = this.termPrev.replace('/home/jiwidi/', '~/')
		document.getElementById("termcli").innerHTML = this.oldInput +
			this.termPrev + this.input;
		while (document.getElementById("term").offsetHeight > this
			.height) {
			this.delFristString();
			document.getElementById("termcli").innerHTML = this
				.oldInput + this.termPrev +
				this.input;
		}
	};
